...
interface Props {
    ...
    data:IProject,
    title?:string,
    type?:'investors' | 'partners' | 'team'
}

const AddInvestorsModal: FC<Props> = ({title,onClose, addInvestors, data,type}) => {
    const [investors,setInvestors] = useState<Array<any>>([])
    const [selectedInvestors,setSelectedInvestors] = useState<Array<Investor>>([])
    const result = useFetch(type ? type === 'investors' ? 'funds' : 'persons' : 'persons')
    ...
    return (
        <Modal title={`Add ${title || 'investors'}`} onClose={onClose} variant='small'>
            <SearchWrapper>
                <SearchIcon />
                <SearchInput
                    type="text"
            ...
            <SubmitButton onClick={confirmInvestors} type='fill'>
                Add {selectedInvestors.length} {title || 'investors'}
            </SubmitButton>
        </Modal>
    );
};
