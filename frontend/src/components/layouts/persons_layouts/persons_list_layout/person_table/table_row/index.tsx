...
import {
    ...
    removeFomoV2Backer,
    toggleFomoV2BackerEralashStatus,
...
const TableRow = ({ openEditModal, project }: { openEditModal: (project: IProject) => void; project: any }) => {
    ...
    const updateStatus = async (value: STATUS_LIST) => {
        setLoading(true)
        await changeFomoV2BackerStatus('persons', project._id, value)
    ...
    const toggleSponsored = async () => {
        setLoading(true)
        await toggleFomoV2BackerSponsoredStatus('persons', project._id)
        reloadWindow()
        setLoading(false)
    }
    ...
    const toggleEralash = async () => {
        setLoading(true)
        await toggleFomoV2BackerEralashStatus('persons', project._id)
        reloadWindow()
        setLoading(false)
    }
    ...
    const deleteProject = useCallback(async () => {
        setLoading(true)
        await removeFomoV2Backer('persons', project._id)
        reloadWindow()
        setLoading(false)
    }, [loading])
    ...
    const toggleRedStatus = useCallback(async () => {
        setLoading(true)
        await toggleFomoV2BackerRedStatus('persons', project._id)
        reloadWindow()
        setLoading(false)
    }, [loading])
    ...
    return (
        <div className={wrapper}>
            <div className={`${rowWrapper} container`}>
                <Link to={`/projects/person/${project._id}`} className={projectWrapper}>
