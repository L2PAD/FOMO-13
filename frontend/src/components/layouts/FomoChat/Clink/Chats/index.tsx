...
import createChat from "../../../../services/messages/createChat";
import getChat from "../../../../services/messages/getChat";
import getChats from "../../../../services/messages/getChats";
import togglePin from "../../../../services/messages/togglePin";
import updateMessages from "../../../../services/messages/updateMessages";
...
const Chats: FC<IProps> = ({
  ...
  const { data, refetch } = useQuery("chats", getChats, {
  ...
  const confirmCreateChat = async (user: IUser | null): Promise<void> => {
    if (!user?._id) {
      return;
    }
  ...
  useEffect(() => {
    ...
    const targetChat = data?.chats?.find((chat: IChat) => chat._id === initialChatId);
    if (targetChat) {
      updateSelectedChat(targetChat);
      updateMessages(targetChat._id!);
      return;
    }

    getChat(initialChatId).then(({ isSuccess, chat }) => {
      if (isSuccess && chat?._id) {
        updateSelectedChat(chat);
        updateMessages(chat._id);
      }
    });
