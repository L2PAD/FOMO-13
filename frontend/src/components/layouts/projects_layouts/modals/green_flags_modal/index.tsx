...
const GreenFlagsModal: FC<Props> = ({onClose}) => {
    ...
    const [flags,setFlags] = useState<Array<IFlag>>(project.greenFlagsList || [])
    const location = useProjectPath() || 'projects'
    ...
    const confirmChanges = async () : Promise<void> => {
        ...
        dispatch(editProject({key:'greenFlagsList',value:flags.filter((item:IFlag) => item.text.length > 0)}))
    }
