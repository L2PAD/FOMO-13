...
import { BADGE_GLYPHS, resolveGlyphKey } from "./badgeGlyphs";
...
interface Props {
  ...
  /** earned => full blue. */
  earned?: boolean;
  /** 0..100 progress toward earning (shown only when not earned & not hidden). */
  progress?: number | null;
  /** hide progress + glyph => grey "?" locked patch. */
  hidden?: boolean;
  title?: string;
  className?: string;
}
...
/**
 * Unified FOMO badge patch. House style: blue hexagon + white glyph.
 * States: earned (blue) · in-progress (muted blue + ring + %) · locked/hidden (grey "?").
 */
const BadgeHex: FC<Props> = ({ icon, size = 56, earned = false, progress = null, hidden = false, title, className }) => {
  const glyphKey = resolveGlyphKey(icon);
  const pct = typeof progress === "number" ? Math.max(0, Math.min(100, Math.round(progress))) : null;
  const showProgressRing = !earned && !hidden && pct != null;
  ...
  return (
    ...
    >
      ...
      {/* % chip for in-progress */}
      {showProgressRing && pct! > 0 && pct! < 100 && (
