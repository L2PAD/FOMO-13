...
async function main(): Promise<void> {
  ...
  try {
    const service = app.get(CoinGeckoMappingBackfillService);
    const result = await service.backfillProjectSourceMaps(options);
...
function parseArgs(argv: string[]): any {
  ...
  for (const arg of argv) {
    ...
    } else if (["limit", "offset", "min-confidence", "minConfidence", "top-skipped-limit", "topSkippedLimit"].includes(key)) {
      const normalizedKey =
        key === "min-confidence"
          ? "minConfidence"
          : key === "top-skipped-limit"
          ? "topSkippedLimit"
          : key;
...
main().catch((error) => {
  console.error(`[coingecko:mapping-backfill] ${error?.message || error}`);
