...
import {
  FomoV2MarketChart7dRenderOptions,
...
type Chart7dRenderTier = MarketDataTier | "all";

interface RunnerArgs extends FomoV2MarketChart7dRenderOptions {
  mode: "dry-run" | "write";
  confirmWrite: boolean;
  tier?: Chart7dRenderTier;
}

export async function main(): Promise<void> {
  const args = parseArgs(process.argv.slice(2));
  logProgress(
    `starting mode=${args.mode} tier=${args.tier || "all"} limit=${formatLimit(args.limit)} offset=${args.offset || 0}`,
  );
  ...
  try {
    const service = app.get(FomoV2MarketProjectChartImageService);
    const result = await service.render7dForEligibleAssets({
...
function parseTier(value: string): Chart7dRenderTier {
...
function formatLimit(value: number | undefined): string {
  if (value === undefined || value === 0) return "all";
  return String(value);
}

function logProgress(message: string): void {
  console.error(`[fomo-v2:market-chart7d-render] ${new Date().toISOString()} ${message}`);
}

if (require.main === module) {
  main().catch((error) => {
    console.error(`[fomo-v2:market-chart7d-render] ${error?.message || error}`);
