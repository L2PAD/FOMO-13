...
import {
  ...
  FomoV2MarketReadModelMaterializeOptions,
} from "../services";

interface RunnerArgs extends FomoV2MarketReadModelMaterializeOptions {
  mode: "dry-run" | "write";
}
...
export async function main(): Promise<void> {
  ...
  logProgress(
    `starting mode=${args.mode} limit=${args.limit} offset=${args.offset || 0} onlyTier=${args.onlyTier || "all"}`,
  );
  ...
  try {
    ...
    const result = await service.materializeFromV2Identity({
      ...args,
...
export function parseArgs(argv: string[]): RunnerArgs {
  ...
  for (const arg of argv) {
    ...
    } else if (key === "only-tier" || key === "onlytier" || key === "tier") {
      args.onlyTier = parseTier(value);
    } else if (key === "examples-limit" || key === "exampleslimit" || key === "examples") {
      args.examplesLimit = parseNonNegativeInteger(value, rawKey);
    } else if (key === "include-unlinked" || key === "includeunlinked") {
      args.includeUnlinked = parseBoolean(value, rawKey);
...
function logProgress(message: string): void {
  console.error(`[fomo-v2:market-read-model-materialize] ${new Date().toISOString()} ${message}`);
}

if (require.main === module) {
  main().catch((error) => {
    console.error(`[fomo-v2:market-read-model-materialize] ${error?.message || error}`);
