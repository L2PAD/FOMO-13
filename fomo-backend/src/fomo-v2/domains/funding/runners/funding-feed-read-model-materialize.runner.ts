...
import {
  FomoV2FundingFeedReadModelMaterializeOptions,
  FomoV2FundingFeedReadModelService,
} from "../services";

interface RunnerArgs extends FomoV2FundingFeedReadModelMaterializeOptions {
  mode: "dry-run" | "write";
}
...
export async function main(): Promise<void> {
  ...
  logProgress(
    `starting mode=${args.mode} limit=${args.limit} offset=${args.offset || 0}`,
  );
  ...
  try {
    const service = app.get(FomoV2FundingFeedReadModelService);
...
function logProgress(message: string): void {
  console.error(`[fomo-v2:funding-feed-read-model-materialize] ${new Date().toISOString()} ${message}`);
}

if (require.main === module) {
  main().catch((error) => {
    console.error(`[fomo-v2:funding-feed-read-model-materialize] ${error?.message || error}`);
