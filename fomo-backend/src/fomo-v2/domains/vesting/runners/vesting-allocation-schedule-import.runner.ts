...
import {
  FomoV2VestingAllocationScheduleImportOptions,
  FomoV2VestingAllocationScheduleImportService,
} from "../services";

type RunnerArgs = FomoV2VestingAllocationScheduleImportOptions & {
  debug: boolean;
  sourceType: string;
  write: boolean;
...
async function main(): Promise<void> {
  ...
  logProgress(
    `starting ${mode} sourceType=${args.sourceType} limit=${
      args.limit ?? "all"
    } debug=${args.debug} writeCandidates=${args.writeCandidates}`
  );
  ...
  try {
    const service = app.get(FomoV2VestingAllocationScheduleImportService);
    const result = await service.run(args);
    console.log(JSON.stringify(result, null, 2));
    assertNoParserImportExecutionErrors(
      result,
      "Vesting allocation/schedule import"
    );
...
export function parseArgs(argv: string[]): RunnerArgs {
  ...
  for (const arg of argv) {
    ...
    if (key === "limit") {
    ...
    } else if (key === "write-candidates" || key === "writecandidates") {
      args.writeCandidates = parseStrictBoolean(value, rawKey);
      ...
      if (!parseStrictBoolean(value, rawKey)) {
        throw new Error("Use --write for vesting import write mode.");
      }
      args.write = false;
    } else {
  ...
  assertParserImportSafety({
    label: "Vesting allocation/schedule import",
    write: args.write,
  ...
  if (args.write && args.limit === undefined && !args.allConfirmed) {
    throw new Error(
      "Vesting allocation/schedule write mode requires --limit or --all-confirmed."
    );
  }
...
function logProgress(message: string): void {
  console.error(
    `[fomo-v2:vesting-allocation-schedule-import] ${new Date().toISOString()} ${message}`
  );
}
...
if (require.main === module) {
  main().catch((error) => {
    console.error(
      `[fomo-v2:vesting-allocation-schedule-import] ${error?.message || error}`
    );
