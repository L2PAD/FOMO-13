...
export interface FundPortfolioAsset {
  _id?: string;
  id?: string;
...
export interface FundPortfolioAssetItem extends FundPortfolioAsset {
  asset?: FundPortfolioAsset;
  project?: FundPortfolioAsset;
  totalRaised?: number;
}

export interface FundPortfolioAssetsPage {
  items: Array<FundPortfolioAssetItem>;
  total: number;
  limit: number;
  offset: number;
  hasMore: boolean;
...
export interface FundPortfolioResponse {
  ok: boolean;
  isSuccess: boolean;
  assets: FundPortfolioAssetsPage;
  items: Array<FundPortfolioAssetItem>;
  portfolioAssets: Array<FundPortfolioAssetItem>;
  total: number;
  limit: number;
  offset: number;
  hasMore: boolean;
  ...
  roundsByCategory: Array<IFundCategoryDistributionItem>;
...
interface FetchFundPortfolioParams {
  offset?: number;
  limit?: number;
...
export default async function fetchFundPortfolio(
  id: string,
  params: FetchFundPortfolioParams = {}
): Promise<FundPortfolioResponse> {
  const offset = Math.max(Number(params.offset) || 0, 0);
  const limit = Math.min(Math.max(Number(params.limit) || 20, 1), 50);

  if (!id || !isFomoV2FundsDetailEnabled()) return emptyResponse(limit, offset);
