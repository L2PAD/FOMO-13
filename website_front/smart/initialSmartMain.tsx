...
import formatSmartDate from "../helpers/formatSmartDate";
...
export async function createPool(
  ...
  const media = mediaInitial?.length
  ...
  try {
    await daiContractWithSigner_pool.createPrePool(
      startTime,
      green_time,
      yellow_time,
      green_zone,
  ...
  try {
    if (isETH) {
      await daiContractWithSigner_pool.createPool(
        pool_id,
        number_to_bigNumber(max_invest, decimalsETH),
        number_to_bigNumber(min_invest_user, decimalsETH),
        number_to_bigNumber(max_invest_user, decimalsETH),
        comission,
        media_comission,
        media,
        isETH
      ...
      await daiContractWithSigner_pool.createPool(
        pool_id,
        number_to_bigNumber(max_invest, decimalsUSDC),
        number_to_bigNumber(min_invest_user, decimalsUSDC),
        number_to_bigNumber(max_invest_user, decimalsUSDC),
        comission,
        media_comission,
        media,
        isETH
...
export async function getPoolInfo(poolID: number, decimals?: number) {
  try {
    ...
    resData.claimed = bigNumber_to_number(
      response.changerate,
...
export async function investSumETH(
  amount: number,
  poolID: number,
  rewards: number
) {
  try {
    ...
    await daiContractWithSigner_pool.Invest(
      numberToBigNumber(amount, decimalsETH),
      poolID,
      refFather,
      { value: ethers.utils.parseUnits(amount.toString(10), "ether") }
    );
...
export async function investSumUSDC(
  amount: number,
  poolID: number,
  rewards: number
) {
  try {
    ...
    await daiContractWithSigner_pool.Invest(
      numberToBigNumber(amount, decimalsUSDC),
      poolID,
      refFather
    );
...
export async function getUserInZone(poolID: number, userAdr: string) {
  try {
    ...
    const index: number = Number(bigNumber_to_number(data, 0));
...
export async function getUserCanInvest(poolID: number, userAdr: string) {
  try {
    ...
    const isCanInvest = await contract_pool.get_user_can_invest(
      poolID,
      userAdr
    );
...
export async function getLeaderBoardData(poolId: number) {
  try {
    ...
    const { data } = await getUserInZone(
      poolId,
      window.ethereum.selectedAddress
    );

    const status = getUserStatusInZone(data);

    const userData: any = boardList.find((user) => {
      return (
...
export async function getUserInvestInfoEndedPool(
  poolId: number,
  userAddress: string
) {
  try {
    ...
    infoBuyNft.purchaseInvest = data?.invest;
...
export async function changeEndedPoolToken(
  addressT: string,
  poolID: number,
  sumT: number,
  decimals: number,
  ...
  try {
    ...
    await daiContractWithSigner_pool.change_endedPoolToken(
      addressT,
      poolID,
      changerate
    );
...
export async function getUserClaimValue(poolID: number, userAddress: string) {
  try {
    // const {claim} = await getClaimData(projectId)

    // const decimals = claim?.decimals

    const { response } = await getPoolInfo(poolID, 18);
    ...
    return {
      ...
      userInvest: data.purchaseInvest,
    };
  } catch (err: any) {
    console.info("err in transaction", err.message);
...
export async function awardsAmount(user_adr: string) {
  try {
    ...
    return {
      success: true,
      awards: bigNumber_to_number(response, decimalsUSDC),
    };
  } catch (err: any) {
    console.info("err", err.message);
...
export async function getUserCanInvestTime(poolID: number, user_adr: string) {
  try {
    ...
    const response = await contract_pool.get_user_can_invest_time(
      poolID,
      user_adr
    );

    const startTime = formatSmartDate(
      new Date(Number(bigNumber_to_number(response[0], 0)) * 1000)
    );
    const endTime = formatSmartDate(
      new Date(Number(bigNumber_to_number(response[1], 0)) * 1000)
    );
