...
export function linkify(text: string) {
  ...
  return parts.map((part, i) => {
    if (part.match(urlRegex)) {
      return (
        <a
          ...
          style={{ color: "var(--main-blue)", textDecoration: "underline" }}
        >
          {part}
        </a>
      );
    ...
    let match;
    while ((match = usernameRegex.exec(part)) !== null) {
      const start = match.index;
      const end = usernameRegex.lastIndex;
      const username = match[1];

      if (start > lastIndex) {
        subParts.push(part.slice(lastIndex, start));
      }

      subParts.push(
        <a
          key={`username-${i}-${start}`}
...
const CustomTwitterAccs = () => {
  const { translateText } = useTranslation();
  const [filterValue, setFilterValue] = useState("all");
  const { data, refetch, isLoading } = useQuery(
    ["custom-twitter-accs", filterValue],
    () => {
      return fetchTwitterAccs(`/user${getQuery()}`);
    },
  ...
  const getQuery = () => {
    switch (filterValue) {
      ...
      case "Followers":
        return "?sortBy=followersCount&order=desc";
      case "Tweets":
        return "?sortBy=tweetCount&order=desc";
      default:
        return ""; // all
    }
  };
  ...
  const addAcc = (item: any): void => {
    if (hiddenAccs.find((inv: any) => inv._id === item._id)) return;
  ...
  const canUpdateParsing = (): boolean => {
    const lastUpdate = localStorage.getItem("lastParsingUpdate");
  ...
  const confirmUpdateParsingAccount = async (
    ...
    if (!canUpdateParsing()) {
      toast.error(
        <div>
          <h3>{translateText("Update not allowed")}</h3>
          <p>{translateText("Please wait at least 1 minute before trying again.")}</p>
        </div>
      );
      return;
    }
    ...
    if (success) {
      localStorage.setItem("lastParsingUpdate", String(Date.now()));
      await refetch();
      toast.success(
        <div>
    ...
    <>
      <Wrapper>
        ...
        <TableHeaderRightWrapper>
          ...
          <button
            className={filterValue === "Newest" ? "selectedSort" : ""}
            onClick={() => setFilterValue("Newest")}
          >
            {translateText("Newest")}
          </button>

          <button
            className={filterValue === "Followers" ? "selectedSort" : ""}
            onClick={() => setFilterValue("Followers")}
          >
            {translateText("Followers")}
          </button>

          <button
            className={filterValue === "Tweets" ? "selectedSort" : ""}
            onClick={() => setFilterValue("Tweets")}
          >
            {translateText("Tweets")}
          </button>
        </TableHeaderRightWrapper>
        <List>
          {isCreatingParsing ? (
                ...
                <ListItem key={i} variant="main">
                  <ListItemHeader>
                    ...
                    <div className="keywords">
                      {formatKeywordsToTags(item.keywords)}
                    </div>
                    <div className="actions">
                      ...
                      <button
                        onClick={() => {
                          setActionModalId(item._id);
                        }}
                      >
                        <EditIcon type="gray" size="big" />
                      </button>
                      <button
...
export default CustomTwitterAccs;
