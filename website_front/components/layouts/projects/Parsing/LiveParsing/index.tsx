...
const LiveParsing: FC<IProps> = ({ type = "public" }) => {
  ...
  const { data, isLoading } = useQuery(
    ["live-keywords", type, debouncedSearch, page, excludedKeywords],
    () =>
      fetchTwitterKeywords(
        `/${type}/keywords/trending?searchValue=${debouncedSearch}&offset=${(page - 1) * limit}&limit=${limit * page}&excludedKeywords=${excludedKeywords.join(",")}`
      ),
