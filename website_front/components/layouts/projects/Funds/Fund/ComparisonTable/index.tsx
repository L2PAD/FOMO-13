...
import { FundComparisonTableRow } from "../../../../../../http/funds/fetchFundComparison";
import { Body, Header, Row, Wrapper } from "./styles";

interface IProps {
  rows?: FundComparisonTableRow[];
  isLoading?: boolean;
}
  ...
  return (
    <Wrapper variant="main">
      <Header>
        ...
        <div title="Portfolio Rounds Raised">Rounds Raised</div>
        <div title="Average Project ROI">Avg. Project ROI</div>
        <div title="Best Investment ROI">Best Inv. ROI</div>
        <div>Volatility</div>
        <div>Risk Level</div>
      </Header>
      <Body>
        {isLoading ? (
          <PlaceholderTable height="58px" rows={6} />
        ) : !rows.length ? (
          <EmptySection />
          ...
          return (
            <Row key={item.id || item.backerId || i}>
              ...
              <div className="bold">
                ${clarifyAmount(item.portfolioRoundsRaised || item.totalRaised || 0)}
              </div>
              <div className="roi">
                <PercentValue
