...
interface ThreadsEntityOverviewProps {
  entityId?: string;
  network?: string;
}

const ThreadsEntityOverview: React.FC<ThreadsEntityOverviewProps> = ({
  entityId,
  network,
}) => {
  ...
  const handleViewOnThreads = () => {
    window.open("https://threads.net/@fomohub", "_blank");
  };
  ...
  const handleViewAllPosts = () => {
    router.push(`/utility/influence/threads/${entityId}/posts`);
  };

  const handleChannelClick = (channelId: string) => {
    router.push(`/utility/influence/threads/${channelId}`);
  };

  return (
    <PageWrapper>
      <PageHeader>
        <PageTitle>Overview • Threads Account</PageTitle>
      </PageHeader>
      ...
      <ContentGrid>
        <Column>
          <EntityHeader
            ...
            threadsScore={entityData.threadsScore}
            // Buttons
            viewButtonLabel="View on Threads"
            onView={handleViewOnThreads}
        ...
        <Column>
          <AISummary network="threads" entityName={entityData.name} />
          <ProductOverview />
          <RelatedAccounts
      ...
      <CompareModal
        ...
        baseActivity={{
          ...
          avgLikes: "1,480",
          avgComments: "86",
          avgViewsVideo: "67,480",
        }}
        baseAudience={{
          directFollowers: {
            label: "Highly engaged followers",
            value: "22%",
            percentage: 22,
          },
          crossPostTraffic: {
            label: "Occasional engagers",
            value: "48%",
            percentage: 48,
          },
          searchHashtags: {
            label: "Silent/low-activity followers",
            value: "24%",
            percentage: 24,
          },
          externalShares: {
            label: "Suspected low-quality/bots",
            value: "6%",
            percentage: 6,
          },
        }}
...
export default ThreadsEntityOverview;
