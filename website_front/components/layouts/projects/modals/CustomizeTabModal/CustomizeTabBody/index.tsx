...
interface Props {
  ...
  description?: string;
  onClose: () => void;
  onConfirm?: (tabs: Array<ICustomTabs & { blockName: string }>) => void;
}

const CustomizeTabBody: FC<Props> = ({
  onClose,
  onConfirm,
  buttonText = "Apply",
  leftButtonText,
  ...
  const [tabs, setTabs] = useState<Array<ICustomTabs>>(CustomTabsDefault);
  ...
  const updateTabs = (key: string): void => {
    ...
    for (let i = 0; i < tabs.length; i++) {
      const tab = tabs[i];
      if (tab.isActive) items.push(tab.key);
    }

    if (items.length >= 8 && !items.includes(key)) {
      toast.error(
        <div>
          <h4>You can't add more tabs!</h4>
          <p>Maximum number of tabs - 8</p>
        </div>
      );
      return;
    }
  ...
  const confirmTabs = (
    items: Array<ICustomTabs & { blockName: string }>
  ): void => {
    const isError = !items.length;
    if (isError) {
      setIsError(true);
    ...
    onConfirm?.(
      items.map((item: ICustomTabs, i: number) => {
        return { ...item, index: i };
      })
    );
  };
  ...
  const groupedTabs = useMemo(() => {
    const group: Record<string, (ICustomTabs & { blockName: string })[]> = {};
    tabs.forEach((tab) => {
  ...
  return (
    <>
      ...
      <Body>
        <LeftColumn>
          {Object.entries(groupedTabs).map(([blockName, items]) => (
        ...
        <RightColumn>
          <TablesWrapper>
            {activeTabs.map((item, index: number) => (
              <TableRow
                draggable
      ...
      <Actions>
        ...
        <Button onClick={() => confirmTabs(activeTabs)} variant="primary">
          {buttonText}
        </Button>
      </Actions>
      <ResetButton>
...
export default CustomizeTabBody;
