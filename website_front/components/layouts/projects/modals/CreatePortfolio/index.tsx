...
import { createPortfolio } from "../../../../../http/portfolio";
...
interface IProps {
  ...
  refetch: () => any
  onModalBack?: () => void;
  onCreated?: (portfolio: IPortfolio) => void;
}
...
const initialPortfolioData: INewPortfolio = {
  name: "",
  description: "",
};

const CreatePortfolio: FC<IProps> = ({ isVisible, onClose, refetch, onModalBack, onCreated }) => {
  ...
  const resetForm = (): void => {
    setStep(1);
    setAssetData(initialPortfolioData);
  ...
  const confirmContinue = async (): Promise<void> => {
    ...
    const result = await createPortfolio(assetData)
    loadingStateHandler(false)

    if (!result.isSuccess || !result.portfolio) {
      toast.error(
        <div>
          <h3>Portfolio Creation Failed</h3>
  ...
  return (
    <MainModal
      ...
      CustomTitle={
        <div className="custom-title portfolio-title">Create New Portfolio</div>
      }
    >
      <Wrapper>
        {userData?.isFullAuth ? (
