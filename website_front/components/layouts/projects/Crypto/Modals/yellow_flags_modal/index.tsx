...
const YellowFlagsModal: FC<Props> = ({
  onClose,
  project,
  updateProjectData,
  v2EntityType,
  ...
  const [flags, setFlags] = useState<Array<IFlag>>(
  ...
  const addFlag = (): void => {
    setFlags((prev) => [...prev, { text: "", link: "", type: false }]);
  };

  const removeFlag = (_flag: IFlag, index: number): void => {
    setFlags((prev) => prev.filter((_item: IFlag, i) => index !== i));
  };
  ...
  const confirmChanges = async (): Promise<void> => {
    if (isV2Flag && v2EntityType && v2EntityId) {
      ...
      if (result.isSuccess) {
        toast.success("Yellow flag sent to moderation");
        onSubmitted?.();
        onClose();
      } else {
        if (
        ...
        toast.error(result.error || "Yellow flag was not submitted");
      }
      return;
    }

    updateProjectData({
      yellowFlagsList: flags.filter((item: IFlag) => item.text.length > 0),
    });
  };
  ...
  const inputsHanlder = useCallback(
    (text: string, index: number) => {
      setFlags((prev) =>
        prev.map((flag: IFlag, i: number) =>
          i === index ? { ...flag, text } : flag
        )
      );
    },
    [flags]
  ...
  return (
    <Modal title="Yellow flags" onClose={onClose} variant="medium">
      {isV2Flag ? (
        <FlagForm>
          <FlagHint>
          ...
          <FlagField>
            ...
            <textarea
              ...
              placeholder="What makes this a yellow flag?"
            />
          </FlagField>
          <FlagField>
...
export default YellowFlagsModal;
