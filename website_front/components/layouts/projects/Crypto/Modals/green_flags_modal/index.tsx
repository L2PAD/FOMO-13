...
const GreenFlagsModal: FC<Props> = ({
  onClose,
  project,
  updateProjectData,
  v2EntityType,
  ...
  const confirmChanges = async (): Promise<void> => {
    if (isV2Flag && v2EntityType && v2EntityId) {
      ...
      if (result.isSuccess) {
        toast.success("Green flag sent to moderation");
        onSubmitted?.();
        onClose();
      } else {
        if (
        ...
        toast.error(result.error || "Green flag was not submitted");
      }
      return;
    }

    updateProjectData({
      greenFlagsList: flags.filter((item: IFlag) => item.text.length > 0),
    });
  };
  ...
  return (
    <Modal title="Green flags" onClose={onClose} variant="medium">
      {isV2Flag ? (
        <FlagForm>
          <FlagHint>
          ...
          <FlagField>
            ...
            <textarea
              ...
              placeholder="What makes this a green flag?"
            />
          </FlagField>
          <FlagField>
