...
import { MessageItem as StyledMessageItem, UserData, Username, Date, MessageBody } from "../styles";
...
const MessageItemComponent: FC<MessageItemProps> = ({
  ...
  const isReported = !!(currentUserId && message.reports?.includes(currentUserId));
  const senderName = getSupportAwareName(message.sender);
  const senderAvatar = getSupportAwareAvatar(
    message.sender,
    message.sender?.photo
      ? imageLoader(message.sender.photo)
  ...
  const handleMessageClick = (event: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (target.closest(".message-actions") || target.closest(".message-popover") || target.closest("a")) {
      return;
    }
  ...
  return (
    ...
    >
      <UserData className="user-data">
        ...
        <Date className="user-data-date">
          {changeDateType(message.date)}
        </Date>
      </UserData>
      <div>
        <MessageBody
