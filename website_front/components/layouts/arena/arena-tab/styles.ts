...
export const LiveBetOdds = styled.span<{ accent: "green" | "red" }>`
  font-weight: var(--font-weight-semibold);
  color: ${({ accent }) => (accent === "green" ? "var(--color-primary)" : "var(--color-danger)")};
`;
...
export const PageNumber = styled.button<{ active?: boolean }>`
  ...
  padding: 4px 6px;
  border-radius: 8px;
  border: 1px solid ${({ active }) => (active ? "var(--color-primary)" : "#dbe2ea")};
  background: ${({ active }) => (active ? "#e8f9f1" : "var(--color-white)")};
  color: ${({ active }) => (active ? "var(--color-primary)" : "#0f172a")};
  font-size: 12px;
  font-weight: var(--font-weight-semibold);
`;
