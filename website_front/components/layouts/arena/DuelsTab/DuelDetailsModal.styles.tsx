...
export const ParticipantCard = styled.div<{
  ...
  border: 2px solid
    ${({ $isWinner, $isLoser }) => {
      if ($isWinner) return "var(--color-primary)";
      if ($isLoser) return "var(--color-danger)";
      return "#F0F2F5";
    }};
  border-radius: 12px;
  padding: 20px;
...
export const DetailValue = styled.div<{
  ...
  color: ${({ $isPositive, $isYes }) => {
    if ($isPositive !== undefined) {
      return $isPositive ? "var(--color-primary)" : "var(--color-danger)";
    }
    if ($isYes !== undefined) {
      return $isYes ? "var(--color-primary)" : "var(--color-danger)";
    }
    return "#0f172a";
  }};
`;
