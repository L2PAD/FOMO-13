...
const RecommendationsModal: FC<Props> = ({ onClose, news, changeHandler }) => {
  ...
  const toggleRecommendation = (item: INews) => {
    if (news.recommendations?.length) {
      news.recommendations.includes(item._id)
        ? changeHandler(
            [
              ...news.recommendations.filter(
                (newsId: string) => newsId !== item._id
              ),
            ],
            "recommendations"
          )
  ...
  return (
    <Modal title="Edit recommendations" onClose={onClose} variant="big">
      <RecGrid>
        {data?.data?.map((newsItem: INews) => {
          return news.recommendations?.includes(newsItem._id) ? (
            <RecItem
              onClick={() => toggleRecommendation(newsItem)}
              key={newsItem._id}
            >
              <img
                src={imageLoader(newsItem.image || "")}
                alt={newsItem.title}
              />
      ...
      <SubmitButton onClick={confirmRecommendations}>Save changes</SubmitButton>
    </Modal>
  );
};
