...
export type BgTypes = "green" | "red" | "yellow" | "blue";

const getBgColor = (type: BgTypes): string => {
  const colors = {
    green:
    ...
    yellow:
      "linear-gradient(90deg, rgba(255, 199, 2, 0) 0%, rgba(255, 199, 2, 0.6) 100%)",
    blue: "linear-gradient(90deg, rgba(79, 133, 189, 0) 0%, rgba(79, 133, 189, 0.6) 100%)",
  };

  return colors[type];
};

const getBorderColor = (type: BgTypes): string => {
  const colors = {
    ...
    yellow: "linear-gradient(90deg, rgba(255, 199, 2, 0) 0%, var(--color-warning) 100%)",
    blue: "linear-gradient(90deg, rgba(79, 133, 189, 0) 0%, #4F85BD 100%)",
  };
...
export const CategoriesProjectContent = styled.div`
  min-height: 0;
  display: flex;
  flex-direction: column;
  gap: 10px;
